...
/* This program sets up two large arrays of size dim and computes the
 ...
 In contrast to the last exercise the second array will be uploaded in each
 iteration.

 In this exercise you should implement the upload of the second array in three
 different ways:
...
int main(int argc, char *argv[]) {
  ...
  for (int iter = 0; iter < 100; ++iter) {
    // Two calculations of dot products per iteration to see the advantage of
    // streams.
    ...
    case 0:
      ...
      for (unsigned int pass = 0; pass < 2; pass++) {
        ...
        // call the kernel
        dotProdKernel<<<blockGrid, threadBlock>>>(
            gpuResult[pass], gpuOperator1[pass], gpuOperator2[pass], dim);
