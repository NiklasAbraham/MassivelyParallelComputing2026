...
int main(int argc, char* argv[])
{
    ...
    for (unsigned int pass = 0; pass < 2; pass++)
    {
        ...
        else // non-pageable memory
        {
            ...
            auto memSize = dim * sizeof(float);
            cudaMallocHost(&(cpuOperator1[pass]), memSize);
            cudaMallocHost(&(cpuOperator2[pass]), memSize);
    ...
    for (int iter = 0; iter < 100; ++iter)
    {
        ...
        case 2:
            ...
            const auto memSize = dim * sizeof(float);
            const auto resSize = MAX_BLOCKS * MAX_THREADS * sizeof(float);
            for (unsigned int pass =0; pass < 2; pass++){
                cudaMemcpyAsync(gpuOperator2[pass], cpuOperator2[pass], memSize, cudaMemcpyHostToDevice, stream[pass]);
                
                dotProdKernel<<<blockGrid, threadBlock, 0, stream[pass]>>>(gpuResult[pass], gpuOperator1[pass], gpuOperator2[pass], dim);

                cudaMemcpyAsync(cpuResult[pass], gpuResult[pass], MAX_BLOCKS * MAX_THREADS * sizeof(float), cudaMemcpyDeviceToHost, stream[pass]);
            }
    ...
    // cleanup GPU memory
    for (int i =0; i < 2; ++i){
        cudaFree(gpuResult[i]);
        cudaFree(gpuOperator1[i]);
        cudaFree(gpuOperator2[i]);
    }
