...
*/
__global__ void reduceScanline(int* dst, const int* src, const int _w, const int _h){
    ...
    unsigned int idx = y * _w + x;
    int original = src[idx];
    
    //write everything into shared mem for faster access (shifted!!)
    tmp[tId] = src[idx];

    __syncthreads(); // wait until everything written
    ...
    /*
    arr = [a b c d e f g h]
           total        (final pass, stored in the last idx)
        /         \
  a+b+c+d       e+f+g+h (second pass)
   /   \         /   \
 a+b   c+d     e+f   g+h  (first pass)
 / \   / \     / \   / \
a   b c   d   e   f g   h
    The array now is 
    a (a + b) c (a + b + c + d) e (e + f) g (total)
    */
    for (unsigned int stride =1; stride < blockDim.x; stride <<= 1){
        ...
        // if (tId % (2*stride) == 0)
        //     dst[pos + 2*stride] = dst[pos + stride]; 
        __syncthreads();
    }

    if (tId == blockDim.x - 1) tmp[tId] = 0; // set root prefix = 0
    __syncthreads();
    
    // now the rest (down pass)

    /*
    from above we now want
    0 a (a+b) (a+b+c) (a+b+c+d) ...
    in the tree above we set first elem to 0
            0
        /       \
  a+b+c+d     e+f+g+h

    for every node parent prefix = P
    left subtree sum = L
    at the root P = 0 and L = a+b+c+d
    so left child prefix = 0
    and right child prefix = a + b + c +d

    in general left child prefex = P, right child prefix = P + L
    */
    for (unsigned int stride = blockDim.x >> 1; stride >= 1; stride >>= 1){
        //we need prefix(left subtree) + prefix(right subtree)
        //prefix(right subtree) includes prefix(left subtree)
        int pos = (tId + 1) * stride * 2 - 1;
        if (pos < blockDim.x){
            int t = tmp[pos - stride];
            //left child = parent prefix
            tmp[pos - stride] = tmp[pos];
            //right child = parent prefix + left sum
            tmp[pos] += t;
        }
        __syncthreads();

        // if (tId % (2 * stride) == 0)
        //     dst[pos + stride] = dst[pos];
    }

    dst[idx] = tmp[tId] + original;
}


__global__ void prefixScanRowSums(int* src, const int _w, const int _h){
    //assumes that it is executed in one block
    unsigned int tId = threadIdx.x; //can use this as row
    extern __shared__ int tmp[];
    tmp[tId] = src[(tId + 1)*_w];

    int original = src[(tId + 1)*_w];
    ...
    src[(tId + 1)*_w] = tmp[tId] + original; // inclusive scan
}

__global__ void addRowOffsets(int* prefix, int w, int h){
    unsigned int tId = threadIdx.x;
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y   = blockIdx.y;

    // last eleemnt in each row is already good
    if (y >= h || x >= w - 1) return;

    unsigned int idx = y * w + x;
    int offset = prefix[y * w];
    prefix[idx + 1] += offset;
}

__global__ void compactKernel(int* featureList, const int* featureImg,
                               const int* prefixSum, int w, int nPix)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x + blockIdx.y * w;
    if (i < nPix && featureImg[i] == 1)
        featureList[prefixSum[i]] = i;
}
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        cudaMemset(gpuPrefixSumShifted, 0, sizeof(float)); //just start with 0
        reduceScanline<<<blockGrid, threadBlock>>>(gpuPrefixSumShifted + 1, gpuFeatureImg, w, h);
        ...
        prefixScanRowSums<<<1, threadBlock, h * sizeof(int)>>>(gpuPrefixSumShifted, w, h);
        ...
        addRowOffsets<<<blockGrid, threadBlock>>>(gpuPrefixSumShifted, w, h);

        compactKernel<<<blockGrid, threadBlock>>>(gpuFeatureList, gpuFeatureImg, gpuPrefixSumShifted, w, nPix);
