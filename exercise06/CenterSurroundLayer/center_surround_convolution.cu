    ...
    torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> O)
{
    ...
    /*
    I - input tensor of shape (N, C_i, H, W)
    O - output tensor of shape (N, Co, H-2, W-2)
    ...
    */
    ...
    for (int b =0; b < N; ++b){
        for (int co =0; co < C_o; ++co){
            scalar_t out =(scalar_t)0;
            for (int ci =0; ci < C_i; ++ci)
                out += I[b][ci][y_o + 1][x_o + 1] * w_c[ci][co] + w_s[ci][co]* S(I, b, ci, y_o + 1, x_o + 1);
            O[b][co][y_o][x_o] = out + w_b[co];
        }
    }
}
             ...
             torch::PackedTensorAccessor32<scalar_t, 2, torch::RestrictPtrTraits> dL_dw_s)
{
    ...
    for (int b =0; b < N; ++b){
        for (int yo =0; yo < Y_o; ++yo){
            for (int xo =0; xo < X_o; ++xo){
                sum_w_c += dL_dO[b][c_o][yo][xo]*I[b][c_i][yo+1][xo+1];
                sum_w_s += dL_dO[b][c_o][yo][xo]*S(I, b, c_i, yo + 1, xo + 1);
            }
        }
    }
    ...
    for (int b =0; b < N; ++b){
        for (int c_i =0; c_i < C_i; ++c_i){
            ...
            for (int c_o =0; c_o < C_o; ++c_o){
                sum += (dL_dO_padded[b][c_o][y_i + 1][x_i + 1]*w_c[c_i][c_o] + w_s[c_i][c_o]*S(dL_dO_padded, b, c_o, y_i + 1, x_i + 1));
            }
