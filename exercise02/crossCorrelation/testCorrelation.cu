...
/* Compute the correlation between two images. Both images are loaded as
 texture. Each thread has a specific offset. It then computes the component-wise
 product of the shifted texture image with texImg1. The result image at location
 (x,y) has the offset (offsX, offsY)
...
__global__ void correlationKernel(float3 *_dst, cudaTextureObject_t texImg1,
  ...
  for (int j = 0; j < _h; ++j) {
    for (int i = 0; i < _w; ++i) {
      ...
      // Attention: Texture access is done via tex2D. Use normalized
      // coordinates!
...
/** this function simply subtracts the average value from an image and scales it
 * to the range of [0,1] from [0,255];
 */
...
int main(int argc, char *argv[]) {
  ...
  // so we normalize both images, i.e. subtract the mean value and scale
  // by 1./255.
  normalize(img1, w, h);
  normalize(img2, w, h);
  ...
  correlationKernel<<<blockGrid, threadBlock>>>((float3 *)gpuResImg, tex1, tex2,
                                                w, h);
