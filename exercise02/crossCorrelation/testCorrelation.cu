                                  ...
                                  cudaTextureObject_t texImg2, int _w, int _h)
{
    ...
    for (int j = 0; j < _h; ++j)
    {
        for (int i = 0; i < _w; ++i)
        {
            ...
            float4 fp = tex2D<float4>(texImg1, (float)i / _w, (float) j / _h);
            float4 gp = tex2D<float4>(texImg2, (float)(i + offsX) / _w, (float)(j + offsY) / _h );
...
int main(int argc, char* argv[])
{
    ...
    cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc(sizeof(float) * 8, sizeof(float) * 8, sizeof(float) * 8, sizeof(float) * 8, cudaChannelFormatKindFloat);
    ...
    const size_t spitch = w * sizeof(float) * 4;

    cudaMemcpy2DToArray(gpuTex1, 0, 0, img3, spitch, w * 4 * sizeof(float), h, cudaMemcpyHostToDevice);
    cudaMemcpy2DToArray(gpuTex2, 0, 0, img4, spitch, w * 4 * sizeof(float), h, cudaMemcpyHostToDevice);
