...
__global__ void calculateHistogram(Bucket* histogram, const Position* inputPositions, int numInputPositions, int bucketsPerAxis){
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
    for (int i = idx; i < numInputPositions; i += (gridDim.x * blockDim.x)){
        int histX = inputPositions[i].x * bucketsPerAxis;
        int histY = inputPositions[i].y * bucketsPerAxis;

        if (histX >= bucketsPerAxis) histX = (bucketsPerAxis - 1);
        if (histY >= bucketsPerAxis) histY = (bucketsPerAxis - 1);

        atomicAdd(&histogram[histY*bucketsPerAxis + histX].numElements, 1);
    }
}


//size is always 16 * 16 = 256 (or bucketsPerAxis * bucketsPerAxis)
//will only do 1 block with 256 threads for now for simplicity
__global__ void prefixSumHistogram(Bucket* buckets){
    unsigned int tId = threadIdx.x;
    __shared__ int shared[256];

    //copy everything into shared mem
    shared[tId] = buckets[tId].numElements;
    ...
    if (tId == blockDim.x - 1) shared[tId] = 0; // set root prefix = 0
    __syncthreads();
    ...
    for (unsigned int stride = blockDim.x >> 1; stride >= 1; stride >>= 1){
        int pos = (tId + 1) * stride * 2 - 1;
        if (pos < blockDim.x){
            int t = shared[pos - stride];
            shared[pos - stride] = shared[pos];
            shared[pos] += t;
        }
        __syncthreads();
    }
    buckets[tId].startIndex = shared[tId]; // replace startIndex with prefix sum (exclusive)
}


__global__ void resetBucketCounters(Bucket* buckets)
{
    int i = threadIdx.x;
...
__global__ void scatterElements(const Position* input, Position* output, int numInputPositions,
                                Bucket* buckets, int bucketsPerAxis, int* scatterCounts){
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;

    for (int i = idx; i < numInputPositions; i += (gridDim.x * blockDim.x)){
        int histX = input[i].x * bucketsPerAxis;
        int histY = input[i].y * bucketsPerAxis;
        if (histX < 0) histX = 0;
        if (histY < 0) histY = 0;
        if (histX >= bucketsPerAxis) histX = (bucketsPerAxis - 1);
        if (histY >= bucketsPerAxis) histY = (bucketsPerAxis - 1);

        int bucketIndex = histY * bucketsPerAxis + histX;
        int offset = atomicAdd(&scatterCounts[bucketIndex], 1);
        int outIndex = buckets[bucketIndex].startIndex + offset;
                                     ...
                                     CudaArray<Bucket>& cudaOutputPositionBuckets)
{
    ...
    int numBuckets = BucketsPerAxis * BucketsPerAxis;
    cudaMemset(cudaOutputPositionBuckets.cudaArray(), 0, cudaOutputPositionBuckets.size() * sizeof(Bucket));

    calculateHistogram<<<256, 256>>>(cudaOutputPositionBuckets.cudaArray(), cudaInputPositions.cudaArray(), cudaInputPositions.size(), BucketsPerAxis);

    CudaArray<int> scatterCounts(numBuckets);
    cudaMemset(scatterCounts.cudaArray(), 0, numBuckets * sizeof(int));

    prefixSumHistogram<<<1, 256>>>(cudaOutputPositionBuckets.cudaArray());
    
    scatterElements<<<256, 256>>>(cudaInputPositions.cudaArray(), cudaOutputPositions.cudaArray(), cudaInputPositions.size(), 
                                  cudaOutputPositionBuckets.cudaArray(), BucketsPerAxis, scatterCounts.cudaArray());
